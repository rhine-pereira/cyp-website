  ...
  return (
    ...
    <div className="max-w-5xl mx-auto p-4 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900">Manage Fundraiser Listings</h1>
        <Link href="/admin/fundraiser" className="text-sky-700 hover:underline">Add New</Link>
      </div>

      <div className="text-sm text-gray-700">Active listings: {activeCount}</div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {items.map((p) => (
            <div key={p.id} className="border rounded-lg p-3 bg-white">
              <div className="aspect-square w-full bg-slate-50 overflow-hidden rounded-md">
                <img src={p.images?.[0]} alt={p.title} className="h-full w-full object-cover" />
              </div>
              <div className="mt-2 font-semibold text-gray-900">{p.title}</div>
              <div className="text-sm text-gray-600 line-clamp-2">{p.description}</div>
              <div className="mt-1 text-sky-700 font-medium">₹{p.price}{p.compareAtPrice ? ` (was ₹${p.compareAtPrice})` : ""}</div>
              <div className="mt-3 flex flex-wrap gap-2">
                <Button size="sm" variant="outline" onClick={() => toggleActive(p)}>
                  {(p.active !== false) ? "Deactivate" : "Activate"}
                </Button>
                <Button size="sm" variant="outline" onClick={() => toggleStock(p)}>
